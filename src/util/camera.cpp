...
void Camera::load(Vec3 pos, Vec3 center, float n_ar, float n_hfov, float n_ap, float n_dist) {

    if(n_ar == 0.0f) n_ar = get_ar();

    float fov = 2.0f * std::atan((1.0f / n_ar) * std::tan(n_hfov / 2.0f));
    fov = Degrees(fov);

    Camera c(Vec2{n_ar, 1.0f});
