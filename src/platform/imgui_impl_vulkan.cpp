...
//  2020-09-07: Vulkan: Added VkPipeline parameter to ImGui_ImplVulkan_RenderDrawData (default to
//  one passed to ImGui_ImplVulkan_Init). 2020-05-04: Vulkan: Fixed crash if initial frame has no
...
//  count. Set ImGui_ImplVulkan_InitInfo::MSAASamples to one of the VkSampleCountFlagBits values to
//  use, default is non-multisampled as before. 2019-05-29: Vulkan: Added support for large mesh
//  (64K+ vertices), enable ImGuiBackendFlags_RendererHasVtxOffset flag. 2019-04-30: Vulkan: Added
//  support for special ImDrawCallback_ResetRenderState callback to reset render state. 2019-04-04:
//  *BREAKING CHANGE*: Vulkan: Added ImageCount/MinImageCount fields in ImGui_ImplVulkan_InitInfo,
//  required for initialization (was previously a hard #define IMGUI_VK_QUEUED_FRAMES 2). Added
...
static void CreateOrResizeBuffer(VkBuffer& buffer, VkDeviceMemory& buffer_memory,
    ...
    g_BufferMemoryAlignment =
        (g_BufferMemoryAlignment > req.alignment) ? g_BufferMemoryAlignment : req.alignment;
...
static void ImGui_ImplVulkan_SetupRenderState(ImDrawData* draw_data, VkPipeline pipeline,
    ...
    // viewport apps.
    {
        ...
        vkCmdPushConstants(command_buffer, g_PipelineLayout, VK_SHADER_STAGE_VERTEX_BIT,
...
static void ImGui_ImplVulkan_CreateDescriptorSetLayout(VkDevice device,
                                                       const VkAllocationCallbacks* allocator) {
    if(g_DescriptorSetLayout) return;
...
bool ImGui_ImplVulkan_CreateDeviceObjects() {
    ...
    ImGui_ImplVulkan_CreatePipeline(v->Device, v->Allocator, v->PipelineCache, g_RenderPass,
                                    v->MSAASamples, &g_Pipeline);
...
void ImGui_ImplVulkan_DestroyDeviceObjects() {
    ImGui_ImplVulkan_InitInfo* v = &g_VulkanInitInfo;
    ImGui_ImplVulkanH_DestroyWindowRenderBuffers(v->Device, &g_MainWindowRenderBuffers,
                                                 v->Allocator);
